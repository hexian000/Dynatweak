...
class Kernel {
    ...
    void setNode(String path, String value, boolean lock) throws IOException {
        if (hasNode(path)) {
            ...
            Log.d("Dynatweak", "node ignored: " + path + " = " + value);
        }
    }
        ...
        if (hasNode(node)) {
            try {
                ...
                if (readNode(node).equals(value))
                    ...
                    Log.w("Dynatweak", "trySetNode mismatch: " + node + " = " + value);
            } catch (IOException e) {
                Log.w("Dynatweak", "trySetNode failed: " + node + " = " + value, e);
                return false;
            }
        } else {
            Log.d("Dynatweak", "node ignored: " + node + " = " + value);
        }
        return false;
    }
